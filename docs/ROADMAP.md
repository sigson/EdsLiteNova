...
2. 🚧 **EncFS** — пофайловое шифрование (не контейнер): своя конфигурация XML,
   ...
   ✅ Совместимость (K1) **подтверждена**: реальная EncFS-папка (настольный `encfs`
   1.9.5, standard) читается байт-в-байт — имена+данные+MAC, chained-IV, вложенные
   каталоги, многоблочные файлы. Эталон + C#-тесты + независимый оракул: см.
   `docs/PHASE-K1-REPORT.md`, `tests/interop-verification/`.
...
- 🚧 **Корпус совместимости**: криптопримитивы уже проверены **official
  ...
  на уровне алгоритмов. ✅ **LUKS1 (AES)** теперь подтверждён на уровне формата:
  реальные заголовки `cryptsetup` 2.7.0 (sha1/256/512, 256/512-бит) — восстановленный
  мастер-ключ совпадает с `luksDump --dump-master-key`. Осталось ⬜ **TrueCrypt/
  VeraCrypt** и serpent/twofish-LUKS (нет утилит/модулей в среде). См.
  `docs/PHASE-K1-REPORT.md`.
